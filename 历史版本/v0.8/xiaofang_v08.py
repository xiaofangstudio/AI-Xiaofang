...
import xiaofang_data_v08 as DATA


VERSION = "0.8"
ENGINE_NAME = "FangPro"
MODEL_NAME = "AI 小方 FangPro 0.8"
STUDIO_NAME = "小方工作室"
...
_MODEL_TIERS = [  # (d, L, H, fnn倍率, 标签, 常驻int8≈)
...
MODEL_TARGET = "1B-class"       # 旗舰愿景: 逼近 1B 参数量
...
# ============================================================
# v0.7: 自学习记忆 (Persistent Self-Learning) → v0.8 沿用并迁移记忆文件
...
# ============================================================
class LearnerMemory:
    MEM_FILE = "xiaofang_memory_v08.json"
    _LEGACY_FILES = ("xiaofang_memory_v07.json",)   # v0.7 旧记忆自动迁移, 不丢已学内容
    ...
    def _save(self):
        p = self._mem_path()
        try:
            data = {"version": "0.8", "seq": self._seq,
...
class XiaoFang:
    ...
    def handle_command(self, cmd):
        ...
        if c == "/help":
            ...
            print(C_REPLY + "🧠 v0.8: 1B-class 模型(≈9.7亿参数·int8量化常驻≤1GB·自动降级算力不减)·跨会话自学习记忆(退出/重进都在)·只存学习成果不存对话与人格。" + C_RESET)
