# -*- coding: utf-8 -*-
# AI 小方 FlphaLit 1.2 —— 快速冒烟测试
# 静默 + 关闭深层思考 + 跑完 os._exit(0)。 重点验证 v1.2 三大改动:
#   ① 深度意图定向(拆词token→矩形矩阵+注意池化+多层精修→意图向量→加权猜猜乐)→治愈牛头不对马嘴
#   ② emoji 只准句尾/标点分句段, 绝不夹在词中间
...
assert M.VERSION == "1.2", ("version wrong", M.VERSION)
xf = M.XiaoFang()
_w("[ok] 装配完成")
_tf = xf.transformer
