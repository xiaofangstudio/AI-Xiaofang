...
def show_startup():
    ...
    version_line = "◆  {} {}  (Deep Reasoning + 1L/1H Transformer)  ◆".format(ENGINE_NAME, VERSION)
    deco_bar = "◆ ═══ ✦ ═══ ❖ ═══ ⬢ ═══ ◈ ═══ ⚡ ═══ ★"
    content = []
    content.append("")
...
class DeepThinkTransformer:
    def __init__(self, vocab_list, d_model=10, n_layers=1, n_heads=1,
...
class ResponseGenerator:
    ...
    def generate(self, query, kb_hits, emo, intent, history):
        ...
        text = query.strip()
        for item in DATA.FIXED_REPLIES:
            kws = item.get("kws", [])
            if not kws:
                # 空关键词 = 默认回答，放最后
                return item["reply"]
...
class XiaoFang:
    def __init__(self):
        ...
        self.transformer = DeepThinkTransformer(
            vocab_list, d_model=10, n_layers=1, n_heads=1, ngram_lm=self.lm)
    ...
    def think(self, user_input, emo, intent, kb_hits):
        ...
        for i, blk in enumerate(tr["blocks"]):
            ...
            parts.append("       L{} 输出 L2={}  1头峰值={}".format(i + 1, _fmt(blk["norm"]), blk["peaks"]))
    ...
    def handle_command(self, cmd):
        ...
        if c == "/help":
            ...
            print(C_REPLY + "💡 1层×1头·d_model=10 | Token计量 | 50条上下文 | 联网整合 | 矩阵运算可视化" + C_RESET)
            return "ok"
        elif c == "/clear":
            self.history.clear()
            self.meter.reset()
