...
import xiaofang_data_v04 as DATA


VERSION = "0.4 正式版"
ENGINE_NAME = "Flphalit"
MODEL_NAME = "AI 小方 Flphalit 0.4"
STUDIO_NAME = "小方工作室"
...
def _typewrite(text, color=C_REPLY, delay=0.035, end="\n"):
    # v0.4.1 打字机效果: 逐字输出, 营造"一点点打出来"的感觉
...
def show_startup():
    ...
    version_line = "◆  {} {}  (Deep Reasoning + 8L/8H Transformer · d_model=512)  ◆".format(ENGINE_NAME, VERSION)
    deco_bar = "◆ ═══ ✦ ═══ ❖ ═══ ⬢ ═══ ◈ ═══ ⚡ ═══ ★"
    content = []
    content.append("")
...
class XiaoFang:
    ...
    def think(self, user_input, emo, intent, kb_hits):
        ...
        elapsed = time.time() - t0   # v0.4.1 本次(前向思考)已实时耗时
        parts.append("  ⏱ 本次思考已耗时 {:.2f} 秒".format(elapsed))
    ...
    def reply(self, user_input, emo, intent, kb_hits):
        ...
        if intent["top"] == "identity" and intent["max_score"] >= 1:
            # v0.4 正式版: 涉及模型/作者/工作室/官网的提问给出完整身份与出处
            if any(m in text for m in ["模型", "作者", "工作室", "开发者", "官网",
                                       "谁做", "谁开发", "网站", "made", "小方工作室"]):
                mline = random.choice([
    ...
    def handle_command(self, cmd):
        ...
        if c == "/help":
            ...
            print(C_REPLY + "💡 {}层×{}头·d_model=512·词库10000+·含英语词库 | 深度权重门控 | 预设应答 | emoji库 | Token计量 | 50条上下文 | 联网整合 | 矩阵运算可视化".format(
    ...
    def process_chat(self, user_input):
        ...
        for p in self.presets:
            if all(k in text for k in p["kws"]):
                answer = p["reply"]
