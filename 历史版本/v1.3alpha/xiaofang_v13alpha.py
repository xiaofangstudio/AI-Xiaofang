...
import xiaofang_data_v13alpha as DATA
VERSION = "1.3 Alpha"
ENGINE_NAME = "FlphaLit"
MODEL_NAME = "AI 小方 FlphaLit 1.3 Alpha"
STUDIO_NAME = "小方工作室"
...
def _read_command(prompt="你: "):
    ...
    def _redraw():
        raw = "".join(_st["buf"])
        if _SYS_BUSY["v"]:                       # 思考中: 只重画输入行, 不画面板
            _st["h"] = 0
            sys.stdout.write("\r\033[K" + C_REPLY + prompt + raw + C_RESET + " ")
            sys.stdout.flush()
            return
        sugs = _matches()
        if _st["h"]:
        ...
        if sugs:
            ...
            lines = [C_HINT + "   ⚡ ↑/↓ 选 · 回车=选中 · Tab 填入 · 继续输=过滤   "
                              "| F1帮助 F2上一个 F3回退 F4新对话 F5停止" + C_RESET]
...
# ============================================================
class LearnerMemory:
    MEM_FILE = "xiaofang_memory_v13alpha.json"
    _LEGACY_FILES = ("xiaofang_memory_v12.json", "xiaofang_memory_v11pro.json",
...
class XiaoFang:
    ...
    def run_with_timeout(self, text, mailbox=None, deferred=None):
        ...
        w.start()
        _SYS_BUSY["v"] = True          # 思考/流式输出期间: 输入框只更新输入行, 不画调色板防撞屏
        t0 = time.time()
        _SPIN = "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"
        while not got.get("done"):
            now = time.time()
            # —— 活动指示行: 只在"近 0.8s 没有流式输出"的空窗才刷新, 不挤断打字机色彩 ——
            if now - _OUT_TICK["t"] > 0.8:
                st = getattr(self, "_stage", "思考中")
                sp = _SPIN[int(now * 4) % len(_SPIN)]
                sys.stdout.write("\r" + C_DEEP + "  " + sp + " 小方正在「" + st + "」… 已 "
                                 + str(int(now - t0)) + "s（这行在走=正常思考；不动即为卡）" + C_RESET + "  ")
                sys.stdout.flush()
            else:
                sys.stdout.write("\r" + " " * 4 + "\r")
                sys.stdout.flush()
            # —— 思考期间仍可打字: 读信箱, /stop 中断当前回答, 其余排队 ——
