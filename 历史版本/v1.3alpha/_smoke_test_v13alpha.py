# -*- coding: utf-8 -*-
# AI 小方 FlphaLit 1.3 Alpha —— 快速冒烟测试
# 静默 + 关闭深层思考 + 跑完 os._exit(0)。 重点验证 v1.3 Alpha 三大改动:
#   ① 变量方程精确求解(修复 "x²=4" 被通用模板"定义/原理/用法"曲解成"X2=4那x等于多")
#   ② 联网搜索全网段 4s 硬超时(修复 "推荐游戏" 联网失败挂 5 分钟假卡死) + 离线兜底给具体推荐
#   ③ 卡死防呆: 纯表情/超短闲聊 → 轻量路径不跑 1B 正演; 活动心跳/阶段/非阻塞输入就绪
#   + "不曲解意图/不滥用知识库"全回归。
import os
...
assert M.VERSION == "1.3 Alpha", ("version wrong", M.VERSION)
xf = M.XiaoFang()
_w("[ok] 装配完成")
_tf = xf.transformer
