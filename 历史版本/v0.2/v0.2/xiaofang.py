...
def show_startup():
    ...
    version_line = "◆  {} {}  (Deep Reasoning + 2L/2H Transformer)  ◆".format(ENGINE_NAME, VERSION)
    deco_bar = "◆ ═══ ✦ ═══ ❖ ═══ ⬢ ═══ ◈ ═══ ⚡ ═══ ★"
    content = []
    content.append("")
...
class DeepThinkTransformer:
    def __init__(self, vocab_list, d_model=16, n_layers=2, n_heads=2,
    ...
    def forward(self, token_ids, trace=False):
        ...
        tr = {
            ...
            "blocks": blocks,
            "out_norm": float(np.linalg.norm(x[-1])),
...
class ResponseGenerator:
    ...
    def generate(self, query, kb_hits, emo, intent, history):
        ...
        # v0.2: 模型太小(d_model=16) + 词库混杂大量大小写英文字母
        # 用 Transformer 直接生成正文 → 输出都是中英字母混合乱码
        seed = self.tok.tokenize(query)[:4]
        if not seed:
            seed = ["我"]
        bias = set(self.tok.tokenize(query))
        if self.tf:
            try:
                gen_tokens = self.tf.generate(
                    seed, bias_tokens=bias, max_tokens=40,
                    temperature=2.5, top_p=0.95)
                body = "".join(gen_tokens)
            except Exception:
    ...
    def _gen_from_kb(self, kb_hits, emo):
        ...
        sents = _split_sents(str(entry["body"]))
        if not sents:
    ...
    def _kb_any_sentence(self, query):
        ...
        best = None
        for entry in DATA.KNOWLEDGE_BASE:
            for s in _split_sents(str(entry["body"])):
...
class XiaoFang:
    def __init__(self):
        ...
        for e in DATA.KNOWLEDGE_BASE:
            kb_words.append(e["title"])
            kb_words.extend(e.get("aliases", []))
        self.tokenizer = Tokenizer(extra_words=kb_words)
        self.lm = NgramLM(self.tokenizer)
        corpus = []
        for e in DATA.KNOWLEDGE_BASE:
            corpus.append(str(e["title"]) + "。" + str(e["body"]))
    ...
    def think(self, user_input, emo, intent, kb_hits):
        ...
        kb_top = kb_hits[0][1]["title"] if kb_hits else None
        kb_score = kb_hits[0][0] if kb_hits else 0.0
        ...
        if kb_hits:
            bias |= set(self.tokenizer.tokenize(kb_hits[0][1]["body"]))
        ...
        if kb_hits:
            for i, (s, e) in enumerate(kb_hits[:3], 1):
                parts.append("  命中{}: 「{}」 Jaccard/覆盖≈{}".format(i, e["title"], s))
        ...
        for i, blk in enumerate(tr["blocks"]):
            ...
            parts.append("       L{} 输出 L2={}  2头峰值={}".format(i + 1, _fmt(blk["norm"]), blk["peaks"]))
    ...
    def reply(self, user_input, emo, intent, kb_hits):
        ...
        # v0.2: 模型太小 + 词库混杂大量字母，绝大部分输出都是中英混合乱码
        # 只有极少数强匹配能命中预设回答，大部分直接交给 Transformer 生成(乱码)
        if intent["top"] == "identity" and intent["max_score"] >= 3:
            return random.choice(DATA.IDENTITY_LINES)
        if intent["top"] == "greet" and intent["max_score"] >= 3:
            return random.choice(DATA.GREET_REPLIES)
        if intent["top"] == "bye" and intent["max_score"] >= 3:
            return random.choice(DATA.BYE_REPLIES)
        # 其余全部走 Transformer 自由生成 → 因为模型太小+字母混杂，输出都是乱码
    ...
    def handle_command(self, cmd):
        ...
        if c == "/help":
            ...
            print(C_REPLY + "💡 2层×2头·d_model=16 | Token计量 | 50条上下文 | 联网整合 | 矩阵运算可视化" + C_RESET)
            return "ok"
        elif c == "/clear":
            self.history.clear()
            self.meter.reset()
