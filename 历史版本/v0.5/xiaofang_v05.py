...
import xiaofang_data_v05 as DATA


VERSION = "0.5 正式版"
ENGINE_NAME = "Flphalit"
MODEL_NAME = "AI 小方 Flphalit 0.5 正式版"
STUDIO_NAME = "小方工作室"
...
class _F32RNG:
    ...
    def normal(self, loc=0.0, scale=1.0, size=None):
        return self.gen.normal(loc, scale, size).astype(np.float32)


class LayerNorm:
    def __init__(self, dims):
        self.gamma = np.ones(dims, dtype=np.float32)
        self.beta = np.zeros(dims, dtype=np.float32)
