# -*- coding: utf-8 -*-
# AI 小方 FlphaLit 1.5 Alpha —— 快速冒烟测试
...
assert M.VERSION == "1.5 Alpha", ("version wrong", M.VERSION)
assert M.MODEL_NAME.endswith("1.5 Alpha"), M.MODEL_NAME
xf = M.XiaoFang()
_w("[ok] 装配完成")
_tf = xf.transformer
...
# ---------- ⑫ v1.5 Alpha: 语言/算法代码生成修复 ----------
# "用JS写个冒泡" → 必须真给 JS 冒泡, 绝不"求和"答非所问
_c = "帮我用JS写个冒泡排序"
emo, intent, kb, _ = _ctx(_c)
assert intent["top"] == "code", ("js bubble not code", _c, intent["top"])
out = xf.reply(_c, emo, intent, kb) or ""
assert "JavaScript" in out, ("js lang not echoed", out[:60])
assert "bubbleSort" in out and "function" in out, ("no js bubble", out[:80])
assert "求和" not in out and "arraySum" not in out and "求和函数" not in out, ("js->sum bug!", out[:80])
_w("[ok] JS冒泡 → 真JS实现(不再求和): ", out.strip().splitlines()[0][:26]); n += 1

_c = "写个冒泡排序给我"
emo, intent, kb, _ = _ctx(_c)
assert intent["top"] == "code", ("bubble not code", _c, intent["top"])
out = xf.reply(_c, emo, intent, kb) or ""
assert "bubble_sort" in out or "冒泡" in out, ("no bubble", out[:80])
assert "求和" not in out, ("bubble->sum", out[:60])
_w("[ok] 冒泡默认 Python 实现(不再求和): ", out.strip().splitlines()[0][:26]); n += 1

_c = "C语言写个素数判断"
emo, intent, kb, _ = _ctx(_c)
assert intent["top"] == "code", ("c prime not code", _c, intent["top"])
out = xf.reply(_c, emo, intent, kb) or ""
assert "C 实现" in out and "isPrime" in out, ("no c prime", out[:80])
_w("[ok] C语言素数 → C 实现: ", out.strip().splitlines()[0][:26]); n += 1

# ---------- ⑬ v1.5 Alpha: 数学 AST (函数/根号/常量/幂混合) ----------
_ast_cases = [
    ("根号16等于多少", "4"),
    ("sqrt(81)是多少", "9"),
    ("sin30度等于多少", "0.5"),
...
for _c, _expect in _ast_cases:
    emo, intent, kb, _ = _ctx(_c)
    assert intent["top"] in ("math", "question", "command"), ("not math-ish", _c, intent["top"])
    out = xf.reply(_c, emo, intent, kb) or ""
    assert ("🧮" in out) and (_expect in out), ("ast math failed", _c, out[:70])
    _w("[ok] 数学AST {} → 命中(含{}):".format(_c, _expect), out.strip().splitlines()[0][:24]); n += 1

# v1.5 系统提示词 / 终止符 / 达标检测 就位
assert "1.5 Alpha" in M.SYSTEM_PROMPT and "小方工作室" in M.SYSTEM_PROMPT, "SYSTEM_PROMPT identity missing"
assert M._terminate_clean("你好") == "你好。", "terminate_clean should append 。"
assert M._terminate_clean("完成！") == "完成！", "terminate_clean must keep existing punct"
assert M._meets_std("还行") and not M._meets_std("  "), "meets_std boundary wrong"
_w("[ok] 系统提示词(身份/厂商/写码/情感/终止/达标) + 终止符/达标检测 就位"); n += 1
