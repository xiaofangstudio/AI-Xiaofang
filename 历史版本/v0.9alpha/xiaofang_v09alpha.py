...
import xiaofang_data_v09alpha as DATA


VERSION = "0.9 Alpha"
ENGINE_NAME = "FangPro"
MODEL_NAME = "AI 小方 FangPro 0.9 Alpha"
STUDIO_NAME = "小方工作室"
...
def _boot(label):
    """v0.9 Alpha: 启动加载进度——实时打印"加载到哪一步", 避免白屏误以为小方卡住."""
    if os.environ.get("XF_QUIET", ""):
        return
    print(C_HINT + "  ⏳ 正在加载中… " + label + " " + C_RESET, flush=True)
...
# ============================================================
class LearnerMemory:
    MEM_FILE = "xiaofang_memory_v09alpha.json"
    _LEGACY_FILES = ("xiaofang_memory_v08pro.json", "xiaofang_memory_v08.json",
...
class XiaoFang:
    def __init__(self):
        ...
        _boot("构建 {} 深度思考引擎 (d_model={}×{}层×{}头)…".format(MODEL_TIER, MODEL_D, MODEL_LAYERS, MODEL_HEADS))
    ...
    def run_with_timeout(self, text):
        ...
        Enter 后立即弹"⏳ 小方正在思考中…"; 后台每 6s 报一次"仍在思考";
        若超过 RESPONSE_TIMEOUT 仍无结果 → 判卡死, 提示重发并安全回到主循环(不永久卡住)."""
        ...
        got = {}
        print(C_DEEP + "⏳ 小方正在思考中…" + C_RESET, flush=True)
        ...
        while not got.get("done"):
            now = time.time()
            if now - last >= 6:
                print(C_DEEP + "  ⏳ 仍在思考… 已 {} 秒 (小方还活着, 请稍候)".format(int(now - t0)) + C_RESET, flush=True)
                last = now
            if now - t0 > RESPONSE_TIMEOUT:
                print()
                print(C_DEEP + "⚠ 这条消息处理太久了，小方目前好像无法处理这个问题 — 请重新发送一下消息。" + C_RESET, flush=True)
                return
            time.sleep(0.15)
        if got.get("err"):
            print(C_DEEP + "⚠ 小方处理这条时出了点小插曲，重新发送一下就好。" + C_RESET, flush=True)
...
if __name__ == "__main__":
    # v0.9 Alpha: 一打开就立即反馈"正在加载中", 不再全屏白屏让用户以为卡住
    print(C_HINT + "🟦 AI 小方 {} 正在启动中… 首次加载需构建 1B+ 模型, 请稍候".format(VERSION) + C_RESET, flush=True)
    XiaoFang().run()