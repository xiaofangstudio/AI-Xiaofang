# -*- coding: utf-8 -*-
import sys, os, platform, subprocess, datetime, random, re, time, json
import numpy as np
...
import xiaofang_data_v08pro as DATA


VERSION = "0.8 Pro"
ENGINE_NAME = "FangPro"
MODEL_NAME = "AI 小方 FangPro 0.8 Pro"
STUDIO_NAME = "小方工作室"
...
def _tier_ok(d, L, f):
    ...
    前向/构建需临时还原为 fp32 → 峰值需小于可用内存."""
    p = _est_params(d, L, f)
    if p > (1 << 30):                       # int8 权重 >1GB → 超预算, 绝不选
        return False
    transient = p * 4 * 1.35                 # fp32 临时还原 + 余量
...
MODEL_TARGET = "1B+"       # 旗舰愿景: 参数量突破 10 亿, 存储(含量化) ≤1GB
...
class Qint:
    ...
    量化一律在 numpy 内完成(与后端无关), 真 GPU 上再由 move_dev() 搬到显存."""
    __slots__ = ("q", "s")
    ...
    def _quantize(w):
        ...
        mx = float(np.max(np.abs(w))) if w.size else 1.0
        if mx < 1e-8:
            return np.zeros_like(w, dtype=np.int8), float(1.0)
        scale = mx / 127.0
        q = np.clip(np.round(w / scale), -127, 127).astype(np.int8)
        return q, float(scale)

    def val(self):
        return _to_dev(self.q.astype(np.float32) * self.s)   # 逐层临时还原到当前后端
...
class DeepThinkTransformer:
    def __init__(self, vocab_list, d_model=MODEL_D, n_layers=MODEL_LAYERS, n_heads=MODEL_HEADS,
        ...
        self.blocks = [TransformerBlock(d_model, d_model * ffn_ratio, n_heads, rng)
...
# ============================================================
# v0.7: 自学习记忆 (Persistent Self-Learning) → v0.8 / v0.8 Pro 沿用并迁移记忆文件
#   · 跨会话持久化: 学到的新词 / 学到的释义摘要写入本地 JSON,
#     退出、重进、甚至清空上下文都依然保留 —— 但绝不保存对话历史/人格/上下文。
#   · 只存"学习成果", 存的是有价值的段落摘要, 不是原文/整段对话。
#   · 三重过滤: ①去重(基础词/已学词/重复标题只累加) ②校验(_plausible 生词合法性)
#     ③价值打分(只留存有信息量的释义, 时长/字数/是否基础词加权)。
#   · 遗忘机制: 学新词的同时按(次数+最近使用)淘汰久不用的旧词(LRU);
#     知识超上限按"价值+新鲜度"淘汰最低者 —— 记忆有界 → 启动只合并不重算, 加载快、内存不膨胀。
# ============================================================
class LearnerMemory:
    MEM_FILE = "xiaofang_memory_v08pro.json"
    _LEGACY_FILES = ("xiaofang_memory_v08.json", "xiaofang_memory_v07.json")  # 旧记忆自动迁移, 不丢已学内容
    ...
    def _save(self):
        p = self._mem_path()
        try:
            data = {"version": "0.8 Pro", "seq": self._seq,
...
class XiaoFang:
    def __init__(self):
        print("\n⏳ 正在启动 AI 小方，加载词库与知识库…", flush=True)
        kb_words = []
        ...
        print("✅ n-gram 语言模型就绪", flush=True)
        ...
        print("⏳ 正在构建深度思考神经网络（约需 20~40 秒）…", flush=True)
        ...
        print("✅ 深度思考模型构建完成，正在装配功能模块…", flush=True)
    ...
    def handle_command(self, cmd):
        ...
        if c == "/help":
            ...
            print(C_REPLY + "🧠 v0.8 Pro: 1B+ 模型(≈10.6亿参数·int8量化常驻≤1GB·自动降级算力不减)·跨会话自学习记忆(退出/重进都在)·只存学习成果不存对话与人格 · 实体直指·答你真正要的。" + C_RESET)
